...
function RemoteQuestsTracker:OnEnable()
	...
	self:SecureHook("WatchFrameAutoQuest_AddPopUp")

	-- Hook for remote quest entry being removed
	self:SecureHook("WatchFrameAutoQuest_ClearPopUp")
	self:UpdateMinion()
end
...
--Events/handlers
function RemoteQuestsTracker:WatchFrameAutoQuest_AddPopUp(...)
...
function RemoteQuestsTracker:WatchFrameAutoQuest_ClearPopUp(...)
	if (blnMinionUpdating == false) then
		self:UpdateMinion()
	end	
end
...
function RemoteQuestsTracker:GetMinionButton()
	...
	if (objButton == nil) then
		...
		objButton = CreateFrame("SCROLLFRAME", strButtonPrefix .. intNumberUsedButtons, UIParent, "WatchFrameAutoQuestPopUpTemplate");		
	end
...
function RemoteQuestsTracker:RecycleMinionButton(objButton)
	...
	objButton.ScrollChild.Flash:Hide();
...
function RemoteQuestsTracker:CreateMinionLayout() 
	...
	fraMinionAnchor:SetScript("OnEnter", function(self) 
		if (dbCore.Main.ShowHelpTooltips == true) then
			...
			GameTooltip:AddLine(L["Drag this to move the Remote Quests minion when it is unlocked.\n"], 1, 1, 1, 1);
	...
	for i=1, numAutoQuestPopUps do
		...
		if (questTitle and questTitle ~= "") then
			...
			if (isComplete and popUpType == "COMPLETE") then
				...
				objButton.ScrollChild.BottomText:Hide();
				objButton.ScrollChild.TopText:SetPoint("TOP", 0, -12);
				objButton.ScrollChild.QuestName:SetPoint("TOP", 0, -32);
				if (objButton.questId and objButton.type=="OFFER") then
					objButton.ScrollChild.Flash:Show();
				end
				objButton.type="COMPLETED";
			elseif (popUpType == "OFFER") then
				...
				objButton.ScrollChild.TopText:SetPoint("TOP", 0, -4);
				objButton.ScrollChild.QuestName:SetPoint("TOP", 0, -24);
				objButton.ScrollChild.Flash:Hide();
				objButton.type="OFFER";
			end
...
function RemoteQuestButton_OnUpdate(frame, timestep)
	local height = 72;
